import { ISearchRegistingEmployee } from './../../store/controls/OperatorsEpic';
...
export default class OperatorApi {
    ...
    static getEmployeesByOperatorId(
        body: ISearchRegistingEmployee,
    ): Observable<any> {
        let api = `${
            OperatorApi.host
        }/${SYSTEM_CONSTANTS.OPERATOR.GET_EMPLOYEES_BY_OPERATOR_ID(body.id)}`;
        if (body.Name) {
            api += `?Name=${body.Name}`;
        }
